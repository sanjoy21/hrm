        ...
        <section class="content">
            <div class="container-fluid">
                ...
                @foreach ($birthday as $bday)
                    @if ($bday->id == Auth::guard('admin')->user()->id && \Carbon\Carbon::parse($bday->dob)->isBirthday())
                        <h4 align='center'>
                    ...
                    @if ($notice_new && $notice_new->expire_date >= \Carbon\Carbon::today()->toDateString())
                        <div class="card mb-3">
                            <div class="card-header bg-primary text-white">
                                ...
                                <div class="float-right"><a href="{{ route('notice', $notice_new->id) }}"
                ...
                @foreach ($applications as $application)
                    <div class="card mb-3">
                        <div class="card-header bg-warning">
                            ...
                            <div class="float-right"><a href="{{ route('application', $application->id) }}"
                                    class="btn btn-sm btn-success">
                                    View
                                </a></div>
                        </div>
                ...
                @foreach ($nocApplications as $noc_application)
                    <div class="card mb-3">
                        <div class="card-header bg-warning">
                            <h5>📢 NOC Application from
                                @foreach ($employees as $employee)
                                    @if ($employee->id == $noc_application->employee_id)
                                        {{ $employee->name }}
                                    @endif
                                @endforeach
                            </h5>
                            <span>Applied on:
                                {{ \Carbon\Carbon::parse($noc_application->created_at)->format('d M, Y') }}</span>
                            <div class="float-right"><a href="{{ route('noc_application', $noc_application->id) }}"
                                    class="btn btn-sm btn-success">
                                    View
                                </a></div>
                        </div>
                ...
                @if (empty(Auth::guard('admin')->user()->blood_group))
                    ...
                    <div class="col-md-4 col-sm-6 col-12">
                        <div class="info-box">
                            ...
                            <div class="info-box-content">
                                ...
                                <span
                                    class="info-box-number">{{ $hq . ' ' . '(M: ' . $hqMale . ' | F: ' . $hqFemale . ')' }}</span>
                            </div>
                            <!-- /.info-box-content -->
                        </div>
                        <!-- /.info-box -->
                    ...
                    <div class="col-md-4 col-sm-6 col-12">
                        <div class="info-box">
                            ...
                            <div class="info-box-content">
                                ...
                                <span
                                    class="info-box-number">{{ $dncc . ' ' . '(M: ' . $dnccMale . ' | F: ' . $dnccFemale . ')' }}</span>
                            </div>
                            <!-- /.info-box-content -->
                        </div>
                        <!-- /.info-box -->
                    ...
                    <div class="col-md-4 col-sm-6 col-12">
                        <div class="info-box">
                            ...
                            <div class="info-box-content">
                                ...
                                <span
                                    class="info-box-number">{{ $ccc . ' ' . '(M: ' . $cccMale . ' | F: ' . $cccFemale . ')' }}</span>
                            </div>
                            <!-- /.info-box-content -->
                        </div>
                        <!-- /.info-box -->
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (checkInBtn) {
                checkInBtn.addEventListener('click', () => {
                    ...
                    if (confirmCheckIn) {
                        getLocationAndSend("{{ route('checkIn') }}", checkInBtn, checkOutBtn, checkInTime, 'checkout');
                    }
                });
            }
            ...
            if (checkOutBtn) {
                checkOutBtn.addEventListener('click', () => {
                    ...
                    if (confirmCheckOut) {
                        getLocationAndSend("{{ route('checkOut') }}", checkOutBtn, null, checkOutTime, null);
                    }
                });
            }
        });
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function getLocationAndSend(url, disableBtn, enableBtn, timeBox, nextMode = null, isCheckOut = false) {
                ...
                if (isProcessing) {
                    showToast('Please wait, your previous request is still processing...', 'warning');
                    // Re-enable the button if it was disabled
                    if (disableBtn) {
                        disableBtn.disabled = false;
                        disableBtn.innerHTML = isCheckOut ?
                            '<i class="fas fa-solid fa-clock"></i> Check Out' :
                            '<i class="fas fa-business-time"></i> Check In';
                    }
                    return;
        ...
        `;
            ...
            // Button click handlers - CHECK IN
            if (checkInBtn) {
                checkInBtn.addEventListener('click', function(e) {
                    e.preventDefault();

                    if (isProcessing) {
                        showToast('Please wait, processing your previous request...', 'warning');
                        return;
                    }

                    if (isOnLeaveToday) {
                        showToast('You cannot check in as you are on leave today.', 'error');
                        return;
                    }
                    ...
                    if (confirmCheckIn) {
                        ...
                        getLocationAndSend("{{ route('checkIn') }}", checkInBtn, checkOutBtn,
                            checkInTime, 'checkout', false);
                    }
                });
            }

            // Button click handlers - CHECK OUT
            if (checkOutBtn) {
                checkOutBtn.addEventListener('click', function(e) {
                    ...
                    if (isOnLeaveToday) {
                        showToast('You cannot check out as you are on leave today.', 'error');
                        return;
                    }
                    ...
                    if (confirmCheckOut) {
                        ...
                        getLocationAndSend("{{ route('checkOut') }}", checkOutBtn, null,
